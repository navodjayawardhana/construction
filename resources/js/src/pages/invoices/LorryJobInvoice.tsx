...
import { getLorryJobInvoice } from '../../services/reportService';
import { useSettings } from '../../contexts/SettingsContext';
import '../../styles/print.css';

const LorryJobInvoice = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await getLorryJobInvoice(id);
    ...
    const getQuantityInfo = (job: LorryJob) => {
        if (job.rate_type === 'per_trip') return { label: 'Number of Trips', value: job.trips ?? 0, unit: 'trip(s)' };
        if (job.rate_type === 'per_km') return { label: 'Distance (KM)', value: job.distance_km ?? 0, unit: 'km' };
        return { label: 'Number of Days', value: job.days ?? 0, unit: 'day(s)' };
    };
            ...
            <div className="print-content bg-white max-w-[210mm] mx-auto p-10 shadow-lg relative" style={{ minHeight: '297mm' }}>
                        ...
                        {(() => {
                            ...
                            return (
                                <svg width="300" height="300" viewBox="0 0 300 300" className="watermark-svg">
                                    ...
                                    {/* Curved text */}
                                    <defs>
                                        <path id="lorryTopArc" d="M 55,150 A 95,95 0 0,1 245,150" fill="none" />
                                        <path id="lorryBottomArc" d="M 245,160 A 95,95 0 0,1 55,160" fill="none" />
                                    </defs>
                                    <text fontSize="14" fill={color} fontWeight="700" fontFamily="Arial, sans-serif" letterSpacing="4">
                                        <textPath href="#lorryTopArc" startOffset="50%" textAnchor="middle">{(settings.business_name || 'CONSTRUCTION CO.').toUpperCase()}</textPath>
                                    </text>
                                    <text fontSize="13" fill={color} fontWeight="600" fontFamily="Arial, sans-serif" letterSpacing="3">
                                        <textPath href="#lorryBottomArc" startOffset="50%" textAnchor="middle">AUTHORIZED</textPath>
                                    </text>
                                </svg>
                            );
                        })()}
                ...
                <div className="flex justify-between items-start mb-8">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Lorry Transport Service</p>
                    </div>
                    <div className="text-right text-sm">
                        <p><span className="font-semibold">Invoice No:</span> LRY-{job.id.substring(0, 8).toUpperCase()}</p>
                ...
                <div className="grid grid-cols-2 gap-8 mb-8">
                    ...
                    <div className="border border-gray-300 rounded p-4">
                        ...
                        {job.worker && <p className="text-sm text-gray-600 mt-2">Driver: {job.worker.name}</p>}
                ...
                <table className="w-full mb-6">
                    <thead>
                        <tr className="bg-gray-100">
                            ...
                            <th className="border border-gray-300 px-4 py-2 text-right text-sm w-40">Details</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                        ...
                        <tr>
                            <td className="border border-gray-300 px-4 py-2 text-sm">{qty.label}</td>
                            <td className="border border-gray-300 px-4 py-2 text-sm text-right">{Number(qty.value).toFixed(2)}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div className="flex justify-end mb-8">
                    <div className="w-80">
                        <div className="flex justify-between py-2 border-b text-sm">
                            ...
                            <span>{formatCurrency(job.rate_amount)} x {Number(qty.value).toFixed(2)} {qty.unit}</span>
                        </div>
...
export default LorryJobInvoice;
